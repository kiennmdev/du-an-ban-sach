...
$dssp = load_all_sach();
$nguoidung = load_all_nguoidung_mo();
    ...
    switch ($act) {
        ...
        case 'nguoidung':
            if (isset($_GET['makhoa'])) {
                $idnd = $_GET['makhoa'];
                delete_nguoidung_mem($idnd);
                header("Location: ?act=nguoidung");
            }
            if (isset($_GET['mamo'])) {
                $idnd = $_GET['mamo'];
                mo_khoa_nguoidung($idnd);
                header("Location: ?act=nguoidung&trangthai=0");
            }
            if (isset($_GET['trangthai']) && $_GET['trangthai'] == 0) {
                $nguoidung = load_all_nguoidung_khoa();
            }
            if (isset($_POST['deleteall'])) {
                $idnd = $_POST['id'];
                if (isset($_GET['trangthai']) && $_GET['trangthai'] == 0) {
                    mo_khoa_nguoidung_multi_item($idnd);
                    header("Location: ?act=nguoidung&trangthai=0");
                } else {
                    delete_nguoidung_mem_multi_item($idnd);
                    header("Location: ?act=nguoidung");
                }
            }
            if (isset($_POST['key'])) {
        ...
        case 'chitietbinhluan':
            ...
            if (isset($_GET['maan'])) {
                $masanpham = $_GET['maan'];
                an_binhluan($masanpham);
                header('location: ' . $_SERVER["HTTP_REFERER"]);
            }
            if (isset($_GET['mahien'])) {
                $masanpham = $_GET['mahien'];
                hien_binhluan($masanpham);
                header('location: ' . $_SERVER["HTTP_REFERER"]);
            }
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $idbt = $_POST['id'];
                an_binhluan_multi_item($idbt);
                header('location: ' . $_SERVER["HTTP_REFERER"]);
            }
